...
recipes = load_data(RECIPES_FILE) if os.path.exists(RECIPES_FILE) else []
...
def update_filters():
    ...
    if st.session_state.selected_category != "All Categories":
        filtered = [r for r in filtered if r.get('category', '') == st.session_state.selected_category]
...
with st.sidebar:
    ...
    # Category filter
    categories = ["All Categories"] + sorted(list(set([r.get('category', '') for r in recipes if r.get('category')])))
...
with col2:
    ...
    if "selected_recipe" in st.session_state and st.session_state.selected_recipe:
        ...
        if ingredients:
            ...
            highest_cost_ingredient = max(ingredients, key=lambda x: float(x.get('total_cost', 0)) if x.get('total_cost') else 0)
